...
struct CS_Output_FB	// Only for Output //
{
	int iColor;	// ARGB : high to low
	float fDepth1stHit; 
};

struct CS_Output_ShadowMap
{
	float fDepthHits[2]; 
	float fOpacity;
};

StructuredBuffer<CS_Output_MultiLayers> g_RWB_PrevLayers: register(t5);	// For Mix //
...
#endif
{
	...
	f3VecDirSamples[0] = normalize(f3VecNormal);
	f3VecDirSamples[1] = float3(0, 0, 1);
	float fLength = 0;
	while((fLength = length(f3VecDirSamples[2] = cross(f3VecDirSamples[0], f3VecDirSamples[1]))) < 0.000001f)
	{
		f3VecDirSamples[1].y += 1;
	}
	f3VecDirSamples[2] = normalize(f3VecDirSamples[2]);
	f3VecDirSamples[1] = normalize(cross(f3VecDirSamples[0], f3VecDirSamples[2]));
	f3VecDirSamples[3] = -f3VecDirSamples[1];
	f3VecDirSamples[4] = -f3VecDirSamples[2];
	// Dont use normalized vector 'cause of its randomness of surface state
	f3VecDirSamples[5] = normalize(f3VecDirSamples[0] + f3VecDirSamples[1]);
	f3VecDirSamples[6] = normalize(f3VecDirSamples[0] + f3VecDirSamples[2]);
	f3VecDirSamples[7] = normalize(f3VecDirSamples[0] + f3VecDirSamples[3]);
	f3VecDirSamples[8] = normalize(f3VecDirSamples[0] + f3VecDirSamples[4]);
	
	f3VecDirSamples[9] = normalize(f3VecDirSamples[5] + f3VecDirSamples[1]);
	f3VecDirSamples[10] = normalize(f3VecDirSamples[6] + f3VecDirSamples[2]);
	f3VecDirSamples[11] = normalize(f3VecDirSamples[7] + f3VecDirSamples[3]);
	f3VecDirSamples[12] = normalize(f3VecDirSamples[8] + f3VecDirSamples[4]);

	f3VecDirSamples[1] = normalize(f3VecDirSamples[0] + f3VecDirSamples[5]);
	f3VecDirSamples[2] = normalize(f3VecDirSamples[0] + f3VecDirSamples[6]);
	f3VecDirSamples[3] = normalize(f3VecDirSamples[0] + f3VecDirSamples[7]);
	f3VecDirSamples[4] = normalize(f3VecDirSamples[0] + f3VecDirSamples[8]);
	
	float3 f3Temp1 = normalize(f3VecDirSamples[1] + f3VecDirSamples[2]);
	float3 f3Temp2 = normalize(f3VecDirSamples[2] + f3VecDirSamples[3]);
	float3 f3Temp3 = normalize(f3VecDirSamples[3] + f3VecDirSamples[4]);
	float3 f3Temp4 = normalize(f3VecDirSamples[4] + f3VecDirSamples[1]);
	f3VecDirSamples[1] = f3Temp1;
	f3VecDirSamples[2] = f3Temp2;
	f3VecDirSamples[3] = f3Temp3;
	f3VecDirSamples[4] = f3Temp4;
...
#ifdef SAMPLE_FROM_BRICKCHUNK
	fSampleValue = vxSampleInBrickChunk(f3PosStartTS, blkObj, tex3DBlock, blkChunk, tex3DChunks);
#else
	...
	for(int i = 1; i < iNumSamples; i++)
	{
		...
		if(blkSkip.iSampleBlockValue > 0)
		{
			for(int k = 0; k < blkSkip.iNumStepSkip; k++, i++)
			{
				...
				{
					float3 f3PosSampleInBlockCTS = vxComputeBrickSamplePosInChunk(f3PosSampleInBlockTS, blkSkip.iSampleBlockValue - 1, blkObj, blkChunk);
					float fSampleValue = vxSampleInBrickChunkWithBlockValue(f3PosSampleInBlockCTS, blkSkip.iSampleBlockValue - 1, blkChunk, tex3DChunks);
					iSampleValue = (int)(fSampleValue * volObj.fSampleValueRange + 0.5f);
				}
				else
				{
					iSampleValue = (int)(vxSampleInBrickChunk(f3PosSampleInBlockTS, blkObj, tex3DBlock, blkChunk, tex3DChunks) * volObj.fSampleValueRange + 0.5f);
				}
				if (iSampleValue >= fMinValidValue)
#else
#ifdef SAMPLER_VOLUME_CCF
				bool bIsPass = false;
...
#endif
				{
					...
					for(int j = 0; j < SURFACEREFINEMENTNUM; j++)
					{
						...
						float3 f3PosBisectionTS = vxTransformPoint(f3PosBisectionWS, volObj.matWS2TS);
#ifdef SAMPLE_FROM_BRICKCHUNK
						// Because Block's 1st hit may be valid point, New block value should be updated to remove grid line artifact appears
						fSampleValue = vxSampleInBrickChunk(f3PosBisectionTS, blkObj, tex3DBlock, blkChunk, tex3DChunks);
						if (fSampleValue >= fMinValidValue)
#else
...
#define VXMIXMAIN	\
	...
	VxRaySegment arrayOutRSA[NUM_MERGE_LAYERS];\
	[loop]	\
	for(i = 0; i < iNumElementsOfCurRSA; i++)	\
	{	\
		stRS_Cur = arrayCurrentRSA[i];\
		...
		{	/*stRS_Cur */\
			...
			{\
				...
				{\
					arrayOutRSA[iCountStoredBuffer] = stIKOut.stIntegrationRS;\
					iCountStoredBuffer++;	\
				}\
				else\
				{\
					VxRaySegment stPrevRS = arrayOutRSA[NUM_MERGE_LAYERS - 1];\
					...
					arrayOutRSA[NUM_MERGE_LAYERS - 1] = stPrevRS;\
				}\
			}\
		...
		{	/*arrayPrevOutRSA 종료*/\
			...
			if(iCountStoredBuffer < NUM_MERGE_LAYERS)\
			{\
				arrayOutRSA[iCountStoredBuffer] = stRS_Cur;\
				iCountStoredBuffer++;	\
			}\
			else\
			{\
	...
	for(i = iIndexOfPrevOutRSA; i < iNumPrevOutRSs; i++)/*arrayPrevOutRSA 만 남아있는 경우*/\
	{\
		stRS_PrevOut = arrayPrevOutRSA[i];	\
		f4IntermixingVisibility += stRS_PrevOut.f4Visibility * (1.f - f4IntermixingVisibility.a);	\
		if(iCountStoredBuffer < NUM_MERGE_LAYERS)\
		{\
			arrayOutRSA[iCountStoredBuffer] = stRS_PrevOut;\
			iCountStoredBuffer++;	\
		}\
		else\
		{\
...
#define VXMIXOUT \
	...
	{\
		...
		{\
			VxRaySegment stRS = arrayOutRSA[i];\
