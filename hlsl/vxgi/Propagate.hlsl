...
void VXGI_Propagate(uint3 id : SV_DispatchThreadID)
{
	...
	// along the march, consistent with the isotropic diffusion model. Weights come from grid_mat
	// (t9), NOT grid_prev.a — the radiance grid's alpha carries the baked per-voxel OBSCURANCE.
	...
	for (int d = 0; d < 14; d++)
	{
		...
		float dist = voxel; // step 1: the immediate neighbour voxel, mip 0
		[unroll]
		for (int s = 0; s < STEPS; s++)
		{
			...
			gather += grid_prev.SampleLevel(g_samplerLinear_clamp, p, lod).rgb * w;
			wsum += w;
	...
	grid_next[id] = float4(radiance, obsc * mat.a);
}
