...
StructuredBuffer<BVHPrimitive> primitiveBuffer : register(t2);
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (DTid.x < primitiveCount)
	{
		...
		do
		{
			...
			BVHNode parent_node = bvhNodeBuffer[nodeIndex];
			...
			BVHNode left_node = bvhNodeBuffer[left_child];
			//BVHNode right_node = bvhNodeBuffer.Load<BVHNode>(right_child * sizeof(BVHNode));
			BVHNode right_node = bvhNodeBuffer[right_child];
			...
			bvhNodeBuffer[nodeIndex] = parent_node;
